...
export class SyncEngine {
  ...
  /**
   * Método genérico de sincronización de entidad
   * @param queryRequired Si es false, la falta de query activa no es un error (útil para entidades opcionales como detalles/pagos)
   */
  private async syncEntity<T>(
    entityType: EntityType,
    options: SyncOptions,
    sendBatchFn: (batch: T[]) => Promise<{
      success: boolean;
      inserted: number;
      updated: number;
      errors: Array<{ index: number; identifier: string; error: string; code: string }>;
    }>,
    queryRequired: boolean = true
  ): Promise<SyncResult> {
    const startTime = Date.now();
    const syncType = options.syncType ?? SyncType.INCREMENTAL;
    ...
    logger.info(
      { entityType, syncType },
      `[SyncEngine] Iniciando sincronización de ${entityType}...`
    );
    ...
    try {
      // 1. Marcar sync como running
      await SyncStateRepo.markSyncAsRunning(entityType);
      ...
      // 3. Obtener query activa para la entidad
      const query = await QueriesRepo.getActiveQueryByEntity(entityType);

      if (!query) {
        // Si la query no es requerida (entidad opcional), omitir sincronización silenciosamente
        if (!queryRequired) {
          logger.info(
            { entityType },
            `[SyncEngine] ⏭️ Omitiendo sincronización de ${entityType}: no hay query activa configurada (entidad opcional)`
          );
          ...
          if (logId) {
            await SyncLogsRepo.updateLog(logId, {
              ...
              recordsSent: 0,
              recordsFailed: 0,
              metadata: { message: `Entidad opcional sin query configurada - omitida` },
            });
          }

          await SyncStateRepo.markSyncAsSuccess(entityType, {
            lastSyncValue: new Date().toISOString(),
            recordCount: 0,
          });
        ...
        // Si la query es requerida, mostrar error detallado
        const allQueries = await QueriesRepo.getAllQueries();
        const queriesForEntity = allQueries.filter(q => q.entityType === entityType);

        let errorMsg = `❌ No hay query activa configurada para ${entityType}.\n\n`;

        if (queriesForEntity.length === 0) {
          errorMsg += `📋 No existe ninguna query para ${entityType}.\n`;
          errorMsg += `   Acción: Ve a Configuración → Consultas y crea una nueva query para ${entityType}.\n`;
        } else {
          errorMsg += `📋 Queries encontradas para ${entityType} (pero ninguna activa):\n`;
          queriesForEntity.forEach(q => {
            errorMsg += `   - ID ${q.id}: "${q.name}" (Activa: ${q.isActive ? 'Sí' : 'No'})\n`;
          });
          errorMsg += `\n   Acción: Ve a Configuración → Consultas y marca una de estas queries como "Activa".\n`;
        }
      ...
      logger.info(
        { queryId: query.id, queryName: query.name, entityType },
        `[SyncEngine] ✅ Query activa encontrada: "${query.name}"`
      );

      // 4. [REMOVED] Field mappings ya no son necesarios - la query debe tener aliases exactos

      // 5. Obtener lastSync value (para sync incremental)
      let lastSyncValue: string | null = null;

      if (!options.fullSync && syncType === SyncType.INCREMENTAL) {
        const syncState = await SyncStateRepo.getSyncState(entityType);
        lastSyncValue = syncState?.lastSyncValue ?? null;
      ...
      // 7. Validar resultados de query contra schema Zod
      logger.info('[SyncEngine] Validando resultados contra schema...');
      ...
      if (!validation.isValid) {
        ...
        await SyncStateRepo.markSyncAsError(entityType, errorMsg);
      ...
      // Validación exitosa - usar datos directamente
      const transformedData = queryResult.rows as T[];
      ...
      // Mostrar muestra de los primeros registros transformados (para debugging)
      if (transformedData.length > 0) {
        logger.debug('\n📝 Muestra de datos transformados (primer registro):');
        logger.debug(JSON.stringify(transformedData[0], null, 2));
      }
      ...
      const batchOptions: BatchProcessorOptions = {
        ...
        saveBatches: true,
        onProgress: this.config.onProgress ? (batchProgress) => {
          // Convertir BatchProgress a ProgressData
          const progressData: ProgressData = {
            entityType,
            current: batchProgress.processedItems,
            total: batchProgress.totalItems,
      ...
      const batchResult = await processBatches(transformedData, sendBatchFn, batchOptions);

      result.recordsSent = batchResult.inserted + batchResult.updated;
      result.recordsFailed = batchResult.errors.length;

      // Reportar resultados del envío
      logger.info('\n========== RESULTADOS DEL ENVÍO ==========');
      logger.info(`Total procesado: ${transformedData.length} registros`);
      logger.info(`✅ Insertados: ${batchResult.inserted}`);
      logger.info(`✅ Actualizados: ${batchResult.updated}`);
      logger.info(`❌ Fallidos: ${batchResult.errors.length}`);
      logger.info(`Batches exitosos: ${batchResult.successfulBatches}/${batchResult.totalBatches}`);
      logger.info('==========================================\n');

      // Si hay errores, mostrar detalles
      if (batchResult.errors.length > 0) {
        logger.error('\n========== ERRORES EN ENVÍO AL GATEWAY ==========');
        logger.error(`Total de errores: ${batchResult.errors.length}\n`);

        // Agrupar errores por código
        const errorsByCode = new Map<string, typeof batchResult.errors>();
        batchResult.errors.forEach(err => {
          const code = err.code || 'UNKNOWN';
          if (!errorsByCode.has(code)) {
            errorsByCode.set(code, []);
          }
          errorsByCode.get(code)!.push(err);
        });

        // Mostrar errores agrupados
        errorsByCode.forEach((errors, code) => {
          logger.error(`Código de error: "${code}" - ${errors.length} caso(s)`);

          // Mostrar hasta 3 ejemplos
          const examples = errors.slice(0, 3);
          examples.forEach((err, idx) => {
            logger.error(`  Ejemplo ${idx + 1}:`);
            logger.error(`    Identificador: ${err.identifier}`);
            logger.error(`    Error: ${err.error}`);
          });

          if (errors.length > 3) {
            logger.error(`  ... y ${errors.length - 3} casos más\n`);
          } else {
            logger.error('');
          }
        });
        ...
        if (errorsByCode.has('VALIDATION_ERROR')) {
          logger.error('💡 SUGERENCIA: Hay errores de validación en el gateway.');
          logger.error('   Los datos enviados no cumplen con el schema del gateway.');
          logger.error('   Revisa que los campos enviados coincidan con los esperados.\n');
        }

        if (errorsByCode.has('PARENT_NOT_FOUND')) {
          logger.error('💡 SUGERENCIA: No se encontró el registro padre (cabecera).');
          logger.error('   Asegúrate de sincronizar primero las cabeceras antes que los detalles/pagos.');
          logger.error('   Verifica que los campos de clave compuesta (erpOperacion, erpFormulario, erpNumero) coincidan.\n');
        }
      }
      ...
      // 10. Agregar errores a retry queue si hay
      if (batchResult.errors.length > 0 && result.recordsSent > 0) {
        logger.info(
          { errorCount: batchResult.errors.length },
          '[SyncEngine] ⚠️ Sincronización parcial: algunos registros fallaron pero otros se procesaron correctamente'
        );
      }

      // 11. Obtener nuevo lastSyncValue
      let newLastSyncValue = lastSyncValue;
      ...
      // 12. Actualizar sync state
      if (result.status === LogStatus.SUCCESS) {
        await SyncStateRepo.markSyncAsSuccess(entityType, {
          lastSyncValue: newLastSyncValue ?? new Date().toISOString(),
          recordCount: result.recordsSent,
        });
      } else if (result.status === LogStatus.PARTIAL) {
        await SyncStateRepo.markSyncAsSuccess(entityType, {
          lastSyncValue: newLastSyncValue ?? new Date().toISOString(),
          recordCount: result.recordsSent,
        });
      ...
      if (logId) {
        ...
        await SyncLogsRepo.updateLog(logId, {
          ...
          metadata: {
            ...
            partialBatches: batchResult.partialBatches,
            totalRecords: transformedData.length,
            batchesStoredInFiles: true, // Indica que los lotes están en archivos
            errors: batchResult.errors.slice(0, 10), // Primeros 10 errores si hay
          },
        });
      }
      ...
      logger.info(
        ...
        `[SyncEngine] ${result.status === LogStatus.SUCCESS ? '✅' : '⚠️'} Sincronización de ${entityType} completada`
      );
      ...
      logger.error(
        { entityType, error },
        `[SyncEngine] ❌ Error en sincronización de ${entityType}`
      );

      // Marcar sync state como error
      await SyncStateRepo.markSyncAsError(
        entityType,
        error instanceof Error ? error.message : String(error)
      );
